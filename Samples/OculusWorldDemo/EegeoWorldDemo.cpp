...
namespace
{
    void UpdateProjection(Matrix4f& p, Eegeo::OVR::OVREegeoCameraController& cameraController)
    {
        ...
        cameraController.SetProjectionMatrix(eegeoProjectionMatrix);
    }
}

EegeoWorldDemoApp::EegeoWorldDemoApp() :
pRender(0),
RenderParams(),
WindowSize(1280,800),
ScreenNumber(0),
...
IsVisionLogging(false)
{
    ...
    pCameraController = new Eegeo::OVR::OVREegeoCameraController(WindowSize.w, WindowSize.h);
    Eegeo::Space::LatLongAltitude eyePosLla = Eegeo::Space::LatLongAltitude::FromDegrees(37.7858,-122.401, 100);
    pCameraController->SetStartLatLongAltitude(eyePosLla);
}
...
int EegeoWorldDemoApp::OnStartup(int argc, const char** argv)
{
    ...
    ovr_TraceMessage(ovrLogLevel_Info, "Eegeo World VR Demo OnStartup");
    
    if (!result)
    {
        OVR::Util::DisplayMessageBox("EegeoWorldDemo", "Unable to initialize LibOVR");
        return 1;
    }
...
bool EegeoWorldDemoApp::SetupWindowAndRendering(int argc, const char** argv)
{
    // *** Window creation
    ...
    StringBuffer title;
    title.AppendFormat("Eegeo World VR Demo %s : %s", graphics, Hmd->ProductName[0] ? Hmd->ProductName : "<unknown device>");
...
void EegeoWorldDemoApp::PopulateOptionMenu()
{
    // For shortened function member access.
    typedef EegeoWorldDemoApp OWD;
...
void EegeoWorldDemoApp::CalculateHmdValues()
{
...
Sizei EegeoWorldDemoApp::EnsureRendertargetAtLeastThisBig(int rtNum, Sizei requestedSize)
{
...
void EegeoWorldDemoApp::OnResize(int width, int height)
{
    ...
    pCameraController->SetTerrainHeightProvider(&(GetEegeoPlatform()->GetTerrainHeightProvider()));
    HmdSettingsChanged = true;
}

void EegeoWorldDemoApp::OnMouseMove(int x, int y, int modifiers)
{
    OVR_UNUSED(y);
...
void EegeoWorldDemoApp::OnKey(OVR::KeyCode key, int chr, bool down, int modifiers)
{
    if (down)
...
Matrix4f EegeoWorldDemoApp::CalculateViewFromPose(const Posef& pose)
{
...
void EegeoWorldDemoApp::OnIdle()
{
    ...
    if (bupdateRenderedView)
    {
        ...
        GetEegeoPlatform()->Update(dt, *pCameraController);
...
bool EegeoWorldDemoApp::FrameNeedsRendering(double curtime)
{
...
void EegeoWorldDemoApp::ApplyDynamicResolutionScaling()
{
...
void EegeoWorldDemoApp::UpdateFrameRateCounter(double curtime)
{
    FrameCounter++;
    TotalFrameCounter++;
...
void EegeoWorldDemoApp::RenderEyeView(ovrEyeType eye)
{
...
void EegeoWorldDemoApp::RenderTextInfoHud(float textHeight)
{
...
void EegeoWorldDemoApp::HmdSensorToggle(OptionVar* var)
{
    if (*var->AsBool())
    {
        EnableSensor = true;
...
void EegeoWorldDemoApp::HmdSettingChangeFreeRTs(OptionVar*)
{
    HmdSettingsChanged = true;
...
void EegeoWorldDemoApp::MultisampleChange(OptionVar*)
{
    HmdSettingChangeFreeRTs();
}

void EegeoWorldDemoApp::CenterPupilDepthChange(OptionVar*)
{
    ovrHmd_SetFloat(Hmd, "CenterPupilDepth", CenterPupilDepthMeters);
}

void EegeoWorldDemoApp::DistortionClearColorChange(OptionVar*)
{
...
void EegeoWorldDemoApp::ResetHmdPose(OptionVar* /* = 0 */)
{
...
void EegeoWorldDemoApp::ProcessDeviceNotificationQueue()
{
    // TBD: Process device plug & Unplug
}

//-----------------------------------------------------------------------------
void EegeoWorldDemoApp::ChangeDisplay ( bool bBackToWindowed, bool bNextFullscreen,
...
void EegeoWorldDemoApp::GamepadStateChanged(const GamepadState& pad)
{
    if (pad.Buttons != 0)
    {   // Dismiss Safety warning with any key.
        ovrHmd_DismissHSWDisplay(Hmd);
...
OVR_PLATFORM_APP(EegeoWorldDemoApp);
